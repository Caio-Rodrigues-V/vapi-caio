...
export class NotificationSender {
  ...
  private buildHtmlEmail(input: NotificationPayload): string {
...
<tr><td style="background:#f9fafb;padding:20px 36px;text-align:center;">
  ...
  async send(input: NotificationPayload): Promise<{ emailSent: boolean; n8nSent: boolean; smsSent?: boolean }> {
    let n8nSent = false;
    let emailSent = false;
  ...
  private async sendSmartRcsSms(input: NotificationPayload, targetPhone: string): Promise<boolean> {
