  ...
  async lookup(cpfInput: string): Promise<DebtLookupResult> {
    ...
    for (const debtor of located) {
      ...
      try {
        ...
        let fase1Data: Record<string, unknown> | null = null;
        try {
          const rawFase1 = await this.getWithRetry<unknown[]>('/calc/retell_fase1.php', {
            tk: calcToken,
            IdCRM: idCrm,
            cpf,
          });
          if (Array.isArray(rawFase1) && rawFase1.length > 0 && typeof rawFase1[0] === 'object') {
            fase1Data = rawFase1[0] as Record<string, unknown>;
        ...
        if (transferirParaOperador) {
          lastResult = {
            ...
            debtorName: String(fase1Data?.NomeDevedor || (debtor as any).nome || ''),
            debtorId,
            calculationId: null,
            nominalAmount: null,
            cashAmount: null,
            firstDueDate: null,
            email: String(fase1Data?.email || (debtor as any).email || '') || null,
            installments: [],
            raw: { fase1: fase1Data },
        ...
        let rawCalculation: unknown = null;
        try {
          const resFase2 = await this.getWithRetry<unknown>('/calc/retell_fase2.php', {
            tk: calcToken,
            IdCRM: idCrm,
            cpf,
          });
        ...
        if (!rawCalculation) {
          rawCalculation = await this.getWithRetry<unknown>('/calc/', {
            tk: calcToken,
            idDev: debtorId,
            cli: client,
          });
        ...
        const isOperatorBlocked = fase1Data?.transferir_para_operador === true ||
          (calculation as any)?.transferir_para_operador === true ||
          ((calculation.Dados as any)?.transferir_para_operador === true) ||
          String((calculation as any).Mensagem || (calculation as any).Msg || '').toLowerCase().includes('bloqueado');
