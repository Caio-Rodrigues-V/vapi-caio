...
function mapStatus(type: string, message: Record<string, any>): 'queued' | 'in_progress' | 'answered' | 'completed' | 'failed' | null {
  const normalized = type.toLowerCase();
  if (normalized.includes('status-update')) {
    const status = String(message.status || message.call?.status || '').toLowerCase();
    if (status === 'queued') return 'queued';
    if (status === 'ringing' || status === 'in-progress') return 'in_progress';
    if (status === 'answered') return 'answered';
    if (status === 'ended' || status === 'completed') return 'completed';
    if (status === 'failed') return 'failed';
  }
  if (normalized === 'end-of-call-report') return 'completed';
  return null;
}
...
export class ProcessVapiWebhook {
  constructor(
    private readonly repository: WebhookRepository,
    private readonly debts?: DebtProvider,
  ) {}

  async execute(payload: Record<string, unknown>): Promise<{ duplicate: boolean; processed: boolean }> {
    ...
    if (!providerCallId) throw new Error('Webhook sem identificador da chamada.');
    ...
    if (mappedStatus) {
      ...
      const customerAskedWhatsapp = customerMessages.some((msg: string) => {
