...
campaignsV2Router.get('/calls/:providerCallId/recording', async (req, res) => {
  ...
  try {
    // 1. Check DB first
    const [rows]: any = await pool.query(
      `SELECT recording_url FROM campaign_calls WHERE provider_call_id = ? LIMIT 1`,
      [providerCallId]
    );
    ...
    try {
      const response = await axios.get(`${vapiBaseUrl}/call/${providerCallId}`, {
        headers: { Authorization: `Bearer ${apiKey}` },
        timeout: 8000,
      });
      ...
      if (audioUrl && audioUrl.startsWith('http')) {
        await pool.query(
          `UPDATE campaign_calls SET recording_url = ? WHERE provider_call_id = ?`,
          [audioUrl, providerCallId]
        ).catch(() => {});
    ...
    const monoResp = await axios.get(`${vapiBaseUrl}/call/${providerCallId}/mono-recording`, {
      headers: { Authorization: `Bearer ${apiKey}` },
      maxRedirects: 0,
...
campaignsV2Router.post('/calls/:providerCallId/terminate', async (req, res) => {
  ...
  try {
    ...
    await axios.delete(
      `${vapiBaseUrl}/call/${providerCallId}`,
      {
        headers: { Authorization: `Bearer ${apiKey}` },
        timeout: 10000,
      }
