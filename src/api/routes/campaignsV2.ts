...
campaignsV2Router.get('/campaigns/diag-skipped-list/:id', async (req, res) => {
  const secret = req.query.secret;
  if (secret !== 'ddm_diag_987') {
    return res.status(401).json({ error: 'Não autorizado' });
  }
  ...
  try {
    const [rows]: any = await pool.query(
      `SELECT id, customer_number, cpf, status, last_error, metadata, updated_at
       FROM campaign_calls
...
campaignsV2Router.get('/campaigns/diag-check-cpf', async (req, res) => {
  const secret = req.query.secret;
  if (secret !== 'ddm_diag_987') {
    return res.status(401).json({ error: 'Não autorizado' });
  }
  ...
  if (!digitsOnly || digitsOnly.length !== 11) return res.status(400).json({ error: 'CPF inválido' });
  ...
  try {
    const [rawDigitsRes, rawFormattedRes] = await Promise.all([
      axios.get('https://ddmacordos.com/calc/localiza_dev.php', { params: { tk: token, cpf: digitsOnly }, timeout: 15000 }).catch(e => ({ data: e.message })),
      axios.get('https://ddmacordos.com/calc/localiza_dev.php', { params: { tk: token, cpf: formattedCpf }, timeout: 15000 }).catch(e => ({ data: e.message })),
    ]);
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (row.status === 'pending') statusText = 'Pendente';
        ...
        const instName = metadata.institution ? ` (${metadata.institution})` : '';
        if (row.last_error === 'already_has_agreement') statusText = `Pulado - Acordo Formalizado${suffix}`;
        else if (row.last_error === 'no_online_agreement') statusText = `Pulado - Acordo Online Bloqueado${suffix}`;
        else if (row.last_error === 'non_uva_institution') statusText = `Pulado - Outra Instituição${instName}`;
        else if (row.last_error === 'no_debt') statusText = metadata.institution ? `Pulado - Outra Instituição${instName}` : `Pulado - Sem Débito${suffix}`;
        else statusText = 'Pulado';
      }
...
campaignsV2Router.get('/vapi/live-analytics', async (_req, res) => {
  try {
    const apiKey = configuredValue(undefined, 'VAPI_API_KEY') || '332987f4-f832-4542-9fd0-76de02bde971';
    if (!apiKey) {
      return res.status(400).json({ error: 'VAPI_API_KEY não configurada' });
    }
        ...
        queries: [
          {
            ...
            timeRange: { start: todayStart.toISOString(), end: new Date().toISOString() },
          },
          {
            table: 'call',
            name: 'lifetime_spend',
            operations: [{ operation: 'sum', column: 'cost' }, { operation: 'count', column: 'id' }],
            timeRange: { start: '2025-01-01T00:00:00.000Z', end: new Date().toISOString() },
          },
        ],
      }),
    ]);
    ...
    const todayData = results.find((q: any) => q.name === 'today_spend')?.result?.[0] || {};
    const lifetimeData = results.find((q: any) => q.name === 'lifetime_spend')?.result?.[0] || {};

    const todayCostUsd = Number(todayData.sumCost || 0);
    const todayCallsCount = Number(todayData.countId || 0);
    const lifetimeCostUsd = Number(lifetimeData.sumCost || 0);
    const lifetimeCallsCount = Number(lifetimeData.countId || 0);
    ...
    return res.json({
      ...
      today: {
        ...
        costBrl: todayCostUsd * usdRate,
        callsCount: todayCallsCount,
        avgCostPerCallBrl: todayCallsCount > 0 ? (todayCostUsd * usdRate) / todayCallsCount : 0,
      },
      lifetime: {
        costUsd: lifetimeCostUsd,
        costBrl: lifetimeCostUsd * usdRate,
        callsCount: lifetimeCallsCount,
        avgCostPerCallBrl: lifetimeCallsCount > 0 ? (lifetimeCostUsd * usdRate) / lifetimeCallsCount : 0,
      },
    });
  } catch (error: any) {
    console.error('[vapi-live-analytics] error:', error.response?.data || error.message);
