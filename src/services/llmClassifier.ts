...
type ClassificacaoType = 'Formaliza' | 'Agendar' | 'Zero';

export interface Classificacao {
  decisao: ClassificacaoType;
  dataAgendamento?: string; // ISO String, se for 'Agendar'
}

/**
 * Analisa a transcrição e classifica a ligação.
 */
export async function classificarLigacao(transcricao: string, falasCliente: string[]): Promise<Classificacao> {
  ...
  try {
    ...
    let decisao: ClassificacaoType = 'Zero';
    if (decisaoCrua.includes('Formaliza')) decisao = 'Formaliza';
    else if (decisaoCrua.includes('Agendar')) decisao = 'Agendar';

    let dataAgendamento: string | undefined;
 ...
 */
async function extrairDataAgendamento(falasCliente: string): Promise<string | undefined> {
  const promptData = `
