...
exports.campaignsV2Router.all('/campaigns/diag-reclassify-campaign/:id', async (req, res) => {
...
exports.campaignsV2Router.get('/campaigns/diag-kill-active-calls', async (req, res) => {
    const secret = req.query.secret;
    if (secret !== 'ddm_diag_987') {
        return res.status(401).json({ error: 'Não autorizado' });
    }
    ...
    try {
       ...
       WHERE campaign_id = ? AND status IN ('reserved','queued','in_progress','answered')`, [campaignId]);
        return res.json({
            success: true,
            campaignId,
...
exports.campaignsV2Router.get('/vapi/live-analytics', async (_req, res) => {
    try {
        ...
        const todayData = results.find((q) => q.name === 'today_spend')?.result?.[0] || {};
        const lifetimeData = results.find((q) => q.name === 'lifetime_spend')?.result?.[0] || {};
