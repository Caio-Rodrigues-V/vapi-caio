...
exports.adminWebhookSimulationRouter = void 0;
...
exports.adminWebhookSimulationRouter = (0, express_1.Router)();
...
exports.adminWebhookSimulationRouter.use(requireAdmin);
exports.adminWebhookSimulationRouter.post('/webhooks/vapi/simulate', async (req, res) => {
    try {
        ...
        const [rows] = await db_1.default.execute(`SELECT id, provider_call_id, customer_number, metadata
       FROM campaign_calls WHERE id = ? LIMIT 1`, [campaignCallId]);
        const callRow = rows[0];
        if (!callRow)
            return res.status(404).json({ error: 'campaign_call não encontrado' });
        ...
        const eventId = String(req.body?.eventId || `simulation:${campaignCallId}:${eventType}:${(0, crypto_1.randomUUID)()}`);
...
//# sourceMappingURL=adminWebhookSimulation.js.map